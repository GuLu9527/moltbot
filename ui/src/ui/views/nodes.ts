  ...
  return html`
    ...
    <section class="card">
      <div class="row" style="justify-content: space-between;">
        <div>
          <div class="card-title">${zhCN.ui.nodes.nodes}</div>
          <div class="card-sub">${zhCN.ui.nodes.nodesSub}</div>
        </div>
        <button class="btn" ?disabled=${props.loading} @click=${props.onRefresh}>
          ${props.loading ? zhCN.ui.nodes.loading : zhCN.ui.nodes.refresh}
        </button>
      </div>
      <div class="list" style="margin-top: 16px;">
        ${props.nodes.length === 0
          ? html`<div class="muted">${zhCN.ui.nodes.noNodesFound}</div>`
...
function renderDevices(props: NodesProps) {
  ...
  return html`
    <section class="card">
      <div class="row" style="justify-content: space-between;">
        <div>
          <div class="card-title">${zhCN.ui.nodes.devices}</div>
          <div class="card-sub">${zhCN.ui.nodes.devicesSub}</div>
        </div>
        <button class="btn" ?disabled=${props.devicesLoading} @click=${props.onDevicesRefresh}>
          ${props.devicesLoading ? zhCN.ui.nodes.loading : zhCN.ui.nodes.refresh}
        </button>
      </div>
      ${props.devicesError
      ...
      <div class="list" style="margin-top: 16px;">
        ${pending.length > 0
          ? html`
              <div class="muted" style="margin-bottom: 8px;">${zhCN.ui.nodes.pending}</div>
              ${pending.map((req) => renderPendingDevice(req, props))}
            `
          : nothing}
        ${paired.length > 0
          ? html`
              <div class="muted" style="margin-top: 12px; margin-bottom: 8px;">${zhCN.ui.nodes.paired}</div>
              ${paired.map((device) => renderPairedDevice(device, props))}
            `
          : nothing}
        ${pending.length === 0 && paired.length === 0
          ? html`<div class="muted">${zhCN.ui.nodes.noPairedDevices}</div>`
          : nothing}
      </div>
    </section>
  `;
...
function renderPendingDevice(req: PendingDevice, props: NodesProps) {
  ...
  const role = req.role?.trim() ? `${zhCN.ui.nodes.role}: ${req.role}` : `${zhCN.ui.nodes.role}: -`;
  const repair = req.isRepair ? ` · ${zhCN.ui.nodes.repair}` : "";
    ...
    <div class="list-item">
      <div class="list-main">
        ...
        <div class="muted" style="margin-top: 6px;">
          ${role} · ${zhCN.ui.nodes.requested} ${age}${repair}
        </div>
      </div>
      <div class="list-meta">
        <div class="row" style="justify-content: flex-end; gap: 8px; flex-wrap: wrap;">
          <button class="btn btn--sm primary" @click=${() => props.onDeviceApprove(req.requestId)}>
            ${zhCN.ui.nodes.approve}
          </button>
          <button class="btn btn--sm" @click=${() => props.onDeviceReject(req.requestId)}>
            ${zhCN.ui.nodes.reject}
          </button>
        </div>
      </div>
    </div>
...
function renderPairedDevice(device: PairedDevice, props: NodesProps) {
  ...
  const roles = `${zhCN.ui.nodes.roles}: ${formatList(device.roles)}`;
  const scopes = `${zhCN.ui.nodes.scopes}: ${formatList(device.scopes)}`;
  ...
  return html`
    <div class="list-item">
      <div class="list-main">
        ...
        ${tokens.length === 0
          ? html`<div class="muted" style="margin-top: 6px;">${zhCN.ui.nodes.tokensNone}</div>`
          : html`
              <div class="muted" style="margin-top: 10px;">${zhCN.ui.nodes.tokens}</div>
...
function renderTokenRow(deviceId: string, token: DeviceTokenSummary, props: NodesProps) {
  const status = token.revokedAtMs ? zhCN.ui.nodes.revoked : zhCN.ui.nodes.active;
  const scopes = `${zhCN.ui.nodes.scopes}: ${formatList(token.scopes)}`;
...
const SECURITY_OPTIONS: Array<{ value: ExecSecurity; label: string }> = [
  { value: "deny", label: zhCN.ui.nodes.deny },
  { value: "allowlist", label: zhCN.ui.nodes.allowlist },
  { value: "full", label: zhCN.ui.nodes.full },
];

const ASK_OPTIONS: Array<{ value: ExecAsk; label: string }> = [
  { value: "off", label: zhCN.ui.nodes.off },
  { value: "on-miss", label: zhCN.ui.nodes.onMiss },
  { value: "always", label: zhCN.ui.nodes.always },
];
  ...
  return html`
    <section class="card">
      <div class="row" style="justify-content: space-between; align-items: center;">
        <div>
          <div class="card-title">${zhCN.ui.nodes.execNodeBinding}</div>
          <div class="card-sub">
            ${zhCN.ui.nodes.execNodeBindingSub}
          </div>
        </div>
        <button
          class="btn"
          ...
          @click=${state.onSave}
        >
          ${state.configSaving ? zhCN.ui.nodes.saving : zhCN.ui.nodes.save}
        </button>
      </div>

      ${state.formMode === "raw"
        ? html`<div class="callout warn" style="margin-top: 12px;">
            ${zhCN.ui.nodes.switchToFormMode}
          </div>`
        : nothing}

      ${!state.ready
        ? html`<div class="row" style="margin-top: 12px; gap: 12px;">
            <div class="muted">${zhCN.ui.nodes.loadConfigToEdit}</div>
            <button class="btn" ?disabled=${state.configLoading} @click=${state.onLoadConfig}>
              ${state.configLoading ? zhCN.ui.nodes.loading : zhCN.ui.nodes.loadConfig}
            </button>
          </div>`
        : html`
            <div class="list" style="margin-top: 16px;">
              <div class="list-item">
                <div class="list-main">
                  <div class="list-title">${zhCN.ui.nodes.defaultBinding}</div>
                  <div class="list-sub">${zhCN.ui.nodes.defaultBindingSub}</div>
                </div>
                <div class="list-meta">
                  <label class="field">
                    <span>${zhCN.ui.nodes.node}</span>
                    <select
                    ...
                    >
                      <option value="" ?selected=${defaultValue === ""}>${zhCN.ui.nodes.anyNode}</option>
                      ${state.nodes.map(
                        (node) =>
                          html`<option
                            value=${node.id}
                  ...
                  ${!supportsBinding
                    ? html`<div class="muted">${zhCN.ui.nodes.noNodesWithSystemRun}</div>`
                    : nothing}
                </div>
              </div>

              ${state.agents.length === 0
                ? html`<div class="muted">${zhCN.ui.nodes.noAgentsFound}</div>`
...
function renderExecApprovals(state: ExecApprovalsState) {
  ...
  return html`
    <section class="card">
      <div class="row" style="justify-content: space-between; align-items: center;">
        <div>
          <div class="card-title">${zhCN.ui.nodes.execApprovals}</div>
          <div class="card-sub">
            ${zhCN.ui.nodes.execApprovalsSub}
          </div>
        </div>
        <button
          class="btn"
          ...
          @click=${state.onSave}
        >
          ${state.saving ? zhCN.ui.nodes.saving : zhCN.ui.nodes.save}
        </button>
      </div>
      ...
      ${!ready
        ? html`<div class="row" style="margin-top: 12px; gap: 12px;">
            <div class="muted">${zhCN.ui.nodes.loadExecApprovals}</div>
            <button class="btn" ?disabled=${state.loading || !targetReady} @click=${state.onLoad}>
              ${state.loading ? zhCN.ui.nodes.loading : zhCN.ui.nodes.loadApprovals}
            </button>
          </div>`
        : html`
...
function renderExecApprovalsTarget(state: ExecApprovalsState) {
  ...
  return html`
    <div class="list" style="margin-top: 12px;">
      <div class="list-item">
        <div class="list-main">
          <div class="list-title">${zhCN.ui.nodes.target}</div>
          <div class="list-sub">
            ${zhCN.ui.nodes.targetSub}
          </div>
        </div>
        <div class="list-meta">
          <label class="field">
            <span>${zhCN.ui.nodes.host}</span>
            <select
            ...
            >
              <option value="gateway" ?selected=${state.target === "gateway"}>${zhCN.ui.nodes.gateway}</option>
              <option value="node" ?selected=${state.target === "node"}>${zhCN.ui.nodes.node}</option>
            </select>
          </label>
          ${state.target === "node"
            ? html`
                <label class="field">
                  ...
                  >
                    <option value="" ?selected=${nodeValue === ""}>${zhCN.ui.nodes.selectNode}</option>
      ...
      ${state.target === "node" && !hasNodes
        ? html`<div class="muted">${zhCN.ui.nodes.noNodesAdvertiseExecApprovals}</div>`
        : nothing}
    </div>
  `;
}
...
function renderExecApprovalsPolicy(state: ExecApprovalsState) {
  ...
  return html`
    <div class="list" style="margin-top: 16px;">
      <div class="list-item">
        <div class="list-main">
          <div class="list-title">${zhCN.ui.nodes.security}</div>
          <div class="list-sub">
            ${isDefaults
              ? zhCN.ui.nodes.securitySub
              : zhCN.ui.nodes.securitySubAgent.replace("{default}", defaults.security)}
          </div>
        </div>
        <div class="list-meta">
          <label class="field">
            <span>${zhCN.ui.nodes.mode}</span>
            <select
              ?disabled=${state.disabled}
              @change=${(event: Event) => {
                const target = event.target as HTMLSelectElement;
            ...
            >
              ${!isDefaults
                ? html`<option value="__default__" ?selected=${securityValue === "__default__"}>
                    ${zhCN.ui.nodes.useDefault} (${defaults.security})
                  </option>`
                : nothing}
      ...
      <div class="list-item">
        <div class="list-main">
          <div class="list-title">${zhCN.ui.nodes.ask}</div>
          <div class="list-sub">
            ${isDefaults ? zhCN.ui.nodes.askSub : zhCN.ui.nodes.askSubAgent.replace("{default}", defaults.ask)}
          </div>
        </div>
        <div class="list-meta">
          <label class="field">
            ...
            >
              ${!isDefaults
                ? html`<option value="__default__" ?selected=${askValue === "__default__"}>
                    ${zhCN.ui.nodes.useDefault} (${defaults.ask})
                  </option>`
                : nothing}
              ${ASK_OPTIONS.map(
                (option) =>
      ...
      <div class="list-item">
        <div class="list-main">
          <div class="list-title">${zhCN.ui.nodes.askFallback}</div>
          <div class="list-sub">
            ${isDefaults
              ? zhCN.ui.nodes.askFallbackSub
              : zhCN.ui.nodes.askFallbackSubAgent.replace("{default}", defaults.askFallback)}
          </div>
        </div>
        <div class="list-meta">
          <label class="field">
            <span>${zhCN.ui.nodes.fallback}</span>
            <select
              ?disabled=${state.disabled}
              @change=${(event: Event) => {
                const target = event.target as HTMLSelectElement;
            ...
            >
              ${!isDefaults
                ? html`<option value="__default__" ?selected=${askFallbackValue === "__default__"}>
                    ${zhCN.ui.nodes.useDefault} (${defaults.askFallback})
                  </option>`
                : nothing}
              ${SECURITY_OPTIONS.map(
                (option) =>
      ...
      <div class="list-item">
        <div class="list-main">
          <div class="list-title">${zhCN.ui.nodes.autoAllowSkillClis}</div>
          <div class="list-sub">
            ${isDefaults
              ? zhCN.ui.nodes.autoAllowSkillClisSub
              : autoIsDefault
                ? zhCN.ui.nodes.autoAllowSkillClisSubDefault.replace("{default}", defaults.autoAllowSkills ? zhCN.ui.nodes.on : zhCN.ui.nodes.off)
                : zhCN.ui.nodes.autoAllowSkillClisSubOverride.replace("{value}", autoEffective ? zhCN.ui.nodes.on : zhCN.ui.nodes.off)}
          </div>
        </div>
        <div class="list-meta">
          <label class="field">
            <span>${zhCN.ui.nodes.enabled}</span>
            <input
              type="checkbox"
              ?disabled=${state.disabled}
...
function renderExecApprovalsAllowlist(state: ExecApprovalsState) {
  ...
  return html`
    <div class="row" style="margin-top: 18px; justify-content: space-between;">
      <div>
        <div class="card-title">${zhCN.ui.nodes.allowlistTitle}</div>
        <div class="card-sub">${zhCN.ui.nodes.allowlistSub}</div>
      </div>
      <button
        class="btn btn--sm"
        ?disabled=${state.disabled}
      ...
      >
        ${zhCN.ui.nodes.addPattern}
      </button>
    </div>
    <div class="list" style="margin-top: 12px;">
      ${entries.length === 0
        ? html`<div class="muted">${zhCN.ui.nodes.noAllowlistEntries}</div>`
...
function renderAllowlistEntry(
  ...
  const lastUsed = entry.lastUsedAt ? formatAgo(entry.lastUsedAt) : zhCN.ui.nodes.never;
  ...
  return html`
    <div class="list-item">
      <div class="list-main">
        <div class="list-title">${entry.pattern?.trim() ? entry.pattern : zhCN.ui.nodes.newPattern}</div>
        <div class="list-sub">${zhCN.ui.nodes.lastUsed}: ${lastUsed}</div>
        ${lastCommand ? html`<div class="list-sub mono">${lastCommand}</div>` : nothing}
        ${lastPath ? html`<div class="list-sub mono">${lastPath}</div>` : nothing}
      </div>
      <div class="list-meta">
        <label class="field">
          <span>${zhCN.ui.nodes.pattern}</span>
          <input
            type="text"
            .value=${entry.pattern ?? ""}
            ?disabled=${state.disabled}
  ...
  return html`
    <div class="list-item">
      <div class="list-main">
        ...
        <div class="list-sub">
          ${agent.isDefault ? zhCN.ui.nodes.defaultAgent : zhCN.ui.nodes.agent} ·
          ${bindingValue === "__default__"
            ? zhCN.ui.nodes.usesDefault.replace("{default}", state.defaultBinding ?? "any")
            : zhCN.ui.nodes.override.replace("{value}", agent.binding ?? "")}
        </div>
      </div>
      <div class="list-meta">
        <label class="field">
          <span>${zhCN.ui.nodes.binding}</span>
          <select
            ?disabled=${state.disabled || !supportsBinding}
            @change=${(event: Event) => {
              const target = event.target as HTMLSelectElement;
    ...
    <div class="list-item">
      <div class="list-main">
        ...
        <div class="chip-row" style="margin-top: 6px;">
          <span class="chip">${paired ? zhCN.ui.nodes.paired : zhCN.ui.nodes.unpaired}</span>
          <span class="chip ${connected ? "chip-ok" : "chip-warn"}">
            ${connected ? zhCN.ui.nodes.connected : zhCN.ui.nodes.offline}
          </span>
