...
function truncatePubkey(pubkey: string | null | undefined): string {
  if (!pubkey) return zhCN.notAvailable;
...
export function renderNostrCard(params: {
    ...
    return html`
      <div class="account-card">
        ...
        <div class="status-list account-card-status">
          ...
          <div>
            <span class="label">${zhCN.ui.channels.publicKey}</span>
    ...
    return html`
      <div style="margin-top: 16px; padding: 12px; background: var(--bg-secondary); border-radius: 8px;">
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px;">
          <div style="font-weight: 500;">${zhCN.ui.channels.profile}</div>
          ${summaryConfigured
            ? html`
                <button
                  class="btn btn-sm"
              ...
              <div class="status-list">
                ${picture
                  ? html`
                    ...
                    `
                  : nothing}
                ${name ? html`<div><span class="label">${zhCN.ui.channels.name}</span><span>${name}</span></div>` : nothing}
                ${displayName
                  ? html`<div><span class="label">${zhCN.ui.channels.displayName}</span><span>${displayName}</span></div>`
                  : nothing}
                ${about
                  ? html`<div><span class="label">${zhCN.ui.channels.about}</span><span style="max-width: 300px; overflow: hidden; text-overflow: ellipsis;">${about}</span></div>`
                  : nothing}
  ...
  return html`
    <div class="card">
      ...
      <div class="card-sub">${zhCN.ui.channels.nostrDesc}</div>
      ${accountCountLabel}
      ...
      ${hasMultipleAccounts
        ...
        : html`
            <div class="status-list" style="margin-top: 16px;">
              <div>
                <span class="label">${zhCN.configured}</span>
                <span>${summaryConfigured ? zhCN.yes : zhCN.no}</span>
              </div>
              <div>
                <span class="label">${zhCN.running}</span>
                <span>${summaryRunning ? zhCN.yes : zhCN.no}</span>
              </div>
              <div>
                <span class="label">${zhCN.ui.channels.publicKey}</span>
              ...
              <div>
                ...
                <span>${summaryLastStartAt ? formatAgo(summaryLastStartAt) : zhCN.notAvailable}</span>
              </div>
            </div>
          `}
      ...
      <div class="row" style="margin-top: 12px;">
        <button class="btn" @click=${() => props.onRefresh(false)}>${zhCN.ui.channels.refresh}</button>
      </div>
    </div>
  `;
}
