...
export function renderChannelConfigForm(props: ChannelConfigFormProps) {
  ...
  if (!normalized) {
    return html`<div class="callout danger">${zhCN.ui.channels.schemaUnavailableUseRaw}</div>`;
  }
  const node = resolveSchemaNode(normalized, ["channels", props.channelId]);
  if (!node) {
    return html`<div class="callout danger">${zhCN.ui.channels.channelConfigSchemaUnavailable}</div>`;
  }
  ...
  return html`
    <div style="margin-top: 16px;">
      ${props.configSchemaLoading
        ? html`<div class="muted">${zhCN.ui.channels.loadingConfigSchema}</div>`
      ...
      <div class="row" style="margin-top: 12px;">
        ...
        >
          ${props.configSaving ? zhCN.saving : zhCN.save}
        </button>
        <button
          class="btn"
          ?disabled=${disabled}
